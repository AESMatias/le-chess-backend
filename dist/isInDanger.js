...
exports.isInDanger = void 0;
const MovementIsValid_1 = require("./MovementIsValid");
const isInDanger = (board, pieceId, currentPosition) => {
    let inDanger = false;
    const currentRow = currentPosition['row'];
    const currentCol = currentPosition['col'];
    const currentDangerPiecesLineal = [];
    const currentDangerPicesDiagonal = [];
    const currentKnightPathPieces = [];
    const pieceColor = pieceId.split('_')[0];
    //TODO: Check the pieces don't go out of the board!
    const recursivePathCheckVertical = (currentPosition, direction) => {
        const currentRow = currentPosition['row'];
        const currentCol = currentPosition['col'];
        ...
        if (currentDangerousPiece !== '') {
            const isValid = (0, MovementIsValid_1.MovementIsValid)(board, currentDangerousPiece, { row: currentRow + direction, col: currentCol }, currentPosition);
            if (!isValid) {
                return true;
            }
        ...
        if (currentDangerousPiece === '') {
            return recursivePathCheckVertical({ row: currentRow + direction, col: currentCol }, direction);
        }
        return false;
    };
    const recursivePathCheckHorizontal = (currentPosition, direction) => {
        const currentRow = currentPosition['row'];
        const currentCol = currentPosition['col'];
        ...
        if (currentDangerousPiece !== '') {
            const isValid = (0, MovementIsValid_1.MovementIsValid)(board, currentDangerousPiece, { row: currentRow, col: currentCol + direction }, currentPosition);
            if (!isValid) {
                return true;
            }
            currentDangerPiecesLineal.push({
        ...
        if (currentDangerousPiece === '') {
            return recursivePathCheckHorizontal({ row: currentRow, col: currentCol + direction }, direction);
        }
        return false;
    };
    const recursivePathCheckDiagonal = (currentPosition, directionRow, directionCol) => {
        const currentRow = currentPosition['row'];
        const currentCol = currentPosition['col'];
        ...
        if (currentDangerousPiece !== '') {
            const isValid = (0, MovementIsValid_1.MovementIsValid)(board, currentDangerousPiece, { row: currentRow + directionRow, col: currentCol + directionCol }, currentPosition);
            // if (isValid) {
            // if(currentDangerousPiece.split('_')[1] === 'Bishop') {
            console.log('se agregooooo ', currentDangerousPiece, 'en la posicion', { row: currentRow + directionRow, col: currentCol + directionCol }, 'luego de revisar la posicion', currentPosition);
            // }
        ...
        if (board[currentRow + directionRow][currentCol + directionCol] === '') {
            return recursivePathCheckDiagonal({ row: currentRow + directionRow, col: currentCol + directionCol }, directionRow, directionCol);
        }
        return false;
    };
    const recursiveKnightCheck = (currentPosition, directionRow, directionCol) => {
    ...
    return ({ inDanger, currentDangerPiecesLineal, currentDangerPicesDiagonal, currentKnightPathPieces });
};
exports.isInDanger = isInDanger;
