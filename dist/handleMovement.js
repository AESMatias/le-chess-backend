...
const checkIsCheckMate_1 = require("./checkIsCheckMate");
const handleMovement = (pieceObject, row, col, board, whoGoes) => {
    ...
    // }
    const isValid = (0, MovementIsValid_1.MovementIsValid)(board, draggedPiece.piece, { row: draggedPiece.row, col: draggedPiece.col }, { row, col });
    ...
    if (draggedPiece && isValid) {
        ...
        if (draggedPiece.piece === 'White_King') {
            kingWhiteRow = row;
            kingWhiteCol = col;
            const returnedDanger = (0, isInDanger_1.isInDanger)(board, 'White_King', { row, col });
        ...
        if (draggedPiece.piece === 'Black_King') {
            kingBlackRow = row;
            kingBlackCol = col;
            const returnedDanger = (0, isInDanger_1.isInDanger)(board, 'Black_King', { row, col });
        ...
        if (draggedPiece.piece.split('_')[1] !== 'King') {
            const returnedDangerWhite = (0, isInDanger_1.isInDanger)(board, 'White_King', { row: kingWhiteRow, col: kingWhiteCol });
            const returnedDangerBlack = (0, isInDanger_1.isInDanger)(board, 'Black_King', { row: kingBlackRow, col: kingBlackCol });
        ...
        const isMate = (0, checkIsCheckMate_1.isCheckMate)(row, col, whoGoes, board, kingWhitePos, kingBlackPos);
