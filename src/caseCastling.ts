import { newRookPosition, newKingPosition, pieceObjectType } from "./Types";
...
export const caseCastling = (board: string[][], pieceId:string, 
    currentPosition: Coords, toPosition: Coords, draggedPiece: pieceObjectType): returnedCastlingObject => {
        ...
        if (pieceColor === 'White' && pieceType === 'King' && currentRow === 0 && currentCol === 4) {
            ...
            if (toRow === 0 && toCol === 0) {
                if (board[0][0] === 'White_Rook') {
                    ...
                    movToDo = {row: 0, col: 3}
                    kingMoveTo = {row: 0, col: 2}
                }
            }
        }

        return {isCastling, movToDo, kingMoveTo}
    }
