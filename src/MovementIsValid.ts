...
import {isAimSameColorPieces} from './isAimSameColorPieces';
...
export const MovementIsValid = (board: string[][], pieceId:string, currentPosition: Coords,
   toPosition: Coords, draggedPiece: pieceObjectType): returnedObject => {


    let isValid = false;
    let castlingEvent = false;
    let movRookToDo = null;
    ...
    if (toCol < 0 || toCol > 7 || toRow < 0 || toRow > 7) {
      ...
      isValid = false
      return {isValid, castlingEvent, movToDo: movRookToDo, kingTo}
    }
  ...
  if (pieceType === 'King') {
    ...
    const {isCastling, movToDo, kingMoveTo} = caseCastling(board, pieceId, currentPosition, toPosition, draggedPiece);
    movRookToDo = movToDo
  ...
  return (
    {isValid, castlingEvent, movToDo: movRookToDo, kingTo}
  )
}
